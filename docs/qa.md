...
The local-only foundation and non-custodial signer architecture have been verified with the credential-free deterministic demo, ESLint, strict TypeScript build, Vitest suite, and production dependency audit. The current suite contains **18 tests** across pairing, internal transfer intent, capability policy, local chat runtime, webhook verification, and signer-boundary behavior. It confirms the project creates no private key, signer, network request, transaction broadcast, live platform call, or enabled buy/sell/stake/bet action.
...
| `pnpm test` | Pass — 6 files / 18 tests | Pairing confirmation, same-platform/expiry/replay/source refusal, recipient opt-in, invalid address/amount refusal, intent idempotency, disabled capabilities, local routing, Discord signature verification, Telegram secret verification, and non-custodial signer-caller/request/freshness/review/fingerprint policy. |
