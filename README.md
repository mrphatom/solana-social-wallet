...
| Routing and fee policy | Pure, `bigint`-safe swap-route and priority-fee evaluators that require independent evidence, full decode evidence, current simulation, caps, and human review. | Quote retrieval, DEX aggregation, RPC, transaction building, parser implementation, wallet connection, signing, submission, priority-fee selection, or token movement. |
| Betting-provider policy | A separate protocol-agnostic capability contract that is deliberately `NOT_AVAILABLE`. | Provider selection, odds/market data, wagering, escrow, settlement, redemption, or compliance/eligibility assertion. |
| Staking, liquidity, and portfolio interfaces | Source-cited future contracts and a disabled capability registry for read-only snapshots, native staking, liquid stake, liquidity positions, and rewards. | Validator/pool recommendation, price/yield prediction, portfolio provider/indexer, stake/delegate/withdraw, liquidity/reward transaction, or asset action. |
| Durable identity and bot hardening | Migration-oriented identity-schema/lifecycle and social-engineering/bot-hijacking defense specifications. | A database, live recovery operation, live bot credential, webhook, queue worker, or public endpoint. |
...
See [the detailed architecture](docs/architecture.md), [social-tip directory design](docs/decentralized-social-tipping-directory.md), [key lifecycle](docs/non-custodial-key-architecture.md), [signing protocol](docs/secure-signing-protocol.md), [secure swap routing](docs/secure-swap-routing-and-fee-policy.md), [transaction fee policy](docs/transaction-fee-policy.md), [review-first social-finance protocol](docs/review-first-social-finance-transaction-protocol.md), [transaction parser/simulation specification](docs/secure-transaction-simulation-and-parsing.md), [cross-platform identity schema](docs/cross-platform-identity-schema-and-lifecycle.md), [staking/liquidity/portfolio interface](docs/non-custodial-staking-liquidity-portfolio-interface.md), [bot-hijacking defense strategy](docs/social-engineering-and-bot-hijacking-defense.md), [routing implementation gates](docs/routing-fee-implementation-gates.md), and [decision records](docs/decisions/). They define boundaries only; they do not activate a financial provider or execution path.
...
| Swap routing and priority fee | Pure evidence evaluator requires matching current fingerprints, independent route/fee domains, bounded integer values, current simulation evidence, program/extension allowlists, and user caps. | A DEX/aggregator/RPC/oracle provider, quote, route, message decode, fee auto-selection, transaction, signature, or broadcast. |
| Buy/sell/stake/bet | Typed `NOT_AVAILABLE` response; separate policy contracts prevent betting or portfolio/staking/liquidity labels from bypassing this state. | Provider, jurisdictional, financial-risk, compliance, custody, consent, parser, and security decisions. |
...
Before any **asset movement**, the owner must separately approve a wallet connection/signing protocol, target cluster, recipient presentation, fee policy, simulation policy, confirmation commitment, timeout handling, user-visible confirmation UX, provider/protocol profile, complete message parser, program/mint/ALT/Token-2022 effect validation, and independent security review. `sendTransaction` can return a signature before the cluster has processed or confirmed it, which is why this foundation never calls “submitted” a completed transfer.[4]
